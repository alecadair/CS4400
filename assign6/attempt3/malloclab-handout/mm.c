...
{
  int offsetandsize = MAX(ALIGN(sizeof(heap_list_node)),ALIGN(size + NODE_SIZE));
...
//	  size_t newsize = (size_to_allocate - sizeof(heap_list_node));
	  ...
	  //void* best_fit_pay = &(best_fit->payload);
	 // best_fit->payload = best_fit_pay;
	  //best_fit->payload += 0x8;
	 // best_fit->payload = best_fit_pay+ 0x8;
	  return best_fit->payload;
//	  best_fit->size = PAGE_ALIGN(offsetandsize +sizeof(heap_list_node)) - sizeof(heap_list_node);// | 1;
//	  list_tail->next = best_fit;
//	  best_fit->prev = list_tail;
//	  list_tail = best_fit;
  }else{
